...
fun main() {
    ...
    val outDir = File("out/braccio/handtohand")
    ...
    val anglesBase = listOf(0, 180)
    val anglesShoulder = listOf(90, 115, 130, 140, 160)
    val anglesElbow = listOf(0, 180)
    val anglesWristVer = listOf(10, 15, 30, 60)
    ...
    val logDir = "data/braccio/handtohand/logs"
    val logFiles = listOf(
        "$logDir/serial_hand_i2c_but1.log",
        "$logDir/serial_hand_i2c_but1_but1.log",
    )
    ...
    if (automaton != null) {
        ...
        hppFile.ensureParentExists().sink().buffer().useWith {
            writeln(
                automaton.toHandToHandControllerCppString(
