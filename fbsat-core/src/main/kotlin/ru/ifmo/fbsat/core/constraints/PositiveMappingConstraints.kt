...
fun Solver.declarePositiveMappingConstraints(
    ...
    declarePositiveMappingConstraintsForRoot()
    ...
    for (v in scenarioTree.activeVertices) {
        ...
        declarePositiveMappingConstraintsForActiveNode(v = v)
    }
    ...
    for (v in scenarioTree.passiveVertices) {
        ...
        declarePositiveMappingConstraintsForPassiveNode(v = v)
    }
    ...
    if (isEncodeReverseImplication) {
        ...
        val mapping: IntVarArray by context
...
private fun Solver.declarePositiveMappingConstraintsForRoot() {
    val mapping: IntVarArray by context
...
private fun Solver.declarePositiveMappingConstraintsForActiveNode(v: Int) {
    val tree: PositiveScenarioTree = context["positiveScenarioTree"]
    val p = tree.parent(v)
    ...
    for (c in 1..C)
        implyAnd(mapping[v] eq c, sequence {
            ...
            // // =====
            // if (tree.outputEvents[o - 1].name == "done") {
            //     // Do nothing
            // } else if (tree.outputEvents[o - 1].name == "packet") {
            //     check(Z == 1)
            //     val z = 1
            //     yield(
            //         algorithmChoice(
...
private fun Solver.declarePositiveMappingConstraintsForPassiveNode(v: Int) {
    val tree: PositiveScenarioTree = context["positiveScenarioTree"]
    val p = tree.parent(v)
    val e = tree.inputEvent(v)
    val u = tree.inputNumber(v)
    ...
    comment("Mapping (positive) propagation for passive node v = $v")
