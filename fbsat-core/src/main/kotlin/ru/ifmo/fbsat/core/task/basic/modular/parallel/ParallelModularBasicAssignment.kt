...
class ParallelModularBasicAssignment(
    ...
    val C: Int,
    val K: Int,
    val color: MultiArray<IntMultiArray>, // [M] : ([V] : 1..C)
        ...
        fun fromRaw(raw: BooleanArray, vars: ParallelModularBasicVariables): ParallelModularBasicAssignment {
            return with(vars) {
                ParallelModularBasicAssignment(
                    ...
                    C = C,
                    K = K,
                    color = MultiArray.create(M) { (m) ->
                        raw.convertIntArray(colorModular[m], V, domain = 1..C) { (m, v) ->
                            error("color[m = $m, v = $v] is undefined")
                        }
                    },
                    transition = MultiArray.create(M) { (m) ->
                        raw.convertIntArray(transitionModular[m], C, K, domain = 1..C) { 0 }
                    },
                    actualTransition = MultiArray.create(M) { (m) ->
                        raw.convertIntArray(actualTransitionModular[m], C, E, U, domain = 1..C) { 0 }
                    },
                    inputEvent = MultiArray.create(M) { (m) ->
                        raw.convertIntArray(inputEventModular[m], C, K, domain = 1..E) { 0 }
                    },
                    outputEvent = MultiArray.create(M) { (m) ->
                        raw.convertIntArray(outputEventModular[m], C, domain = 1..O) { 0 }
                    },
                    outputVariableModule = raw.convertIntArray(outputVariableModule, Z, domain = 1..M) { (z) ->
                    ...
                    algorithm = MultiArray.create(M) { (m) ->
                        MultiArray.create(C) { (c) ->
                            BinaryAlgorithm(
                                ...
                                algorithm0 = BooleanArray(Z) { z -> raw[algorithm0Modular[m][c, z + 1] - 1] },
                                algorithm1 = BooleanArray(Z) { z -> raw[algorithm1Modular[m][c, z + 1] - 1] }
                            ) as Algorithm
                        }
                    },
                    rootValue = MultiArray.create(M) { (m) ->
                        raw.convertBooleanArray(rootValueModular[m], C, K, U)
                    },
                    firstFired = MultiArray.create(M) { (m) ->
                        raw.convertIntArray(firstFiredModular[m], C, U, domain = 1..K) { 0 }
                    },
                    notFired = MultiArray.create(M) { (m) ->
                        raw.convertBooleanArray(notFiredModular[m], C, U, K)
                    }
                )
            }
        }
